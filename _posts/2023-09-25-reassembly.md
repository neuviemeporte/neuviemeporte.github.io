...
The first step to achieve that would be to take the disassembly generated by IDA and try to reassemble it into an EXE file that is identical with the original. In theory, this should work because a disassembly is essentially a listing of all the bytes in the file. But due to assembler syntax quirks, instruction encoding differences, segment layout, padding and alignment etc., the result is not guaranteed, which is why I have not tried to do it in earnest until now.
...
The encoding of some arithmetic instructions involving the `AX` register (`add/sub/and/cmp ax, <immediate>`) differs between the original and what UASM generated. Don't know if there's a way to force the encoding needed with special syntax, so this was solved again by changing the instructions into literal byte sequences such as `db 3Dh, dw <immediate> ; cmp ax, <immediate>`. The same solution was used for a single instance of an `or di, 2`.
