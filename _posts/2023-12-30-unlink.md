---
layout: post
title: First steps in unlinking
category: f15-se2
---
...
Looking good, wonder what will happen when I link in the actual libc startup code? For that I need to remove the `__acrtused` tag and either get rid of the `/NOD` option, or spell the name of the appropriate library file (`slibce.lib` for (s)mall memory model and software (e)mulated floating point) on the linker command line - I choose the latter.
...
md5sum build-f15-se2/hello.exe
03f9b3c84f3d985bb6a54070a79c8dd6  build-f15-se2/hello.exe
</pre>
...
15 routines (and a "chunk" which in IDA speak is a disconnected part of a routine) made it into the executable. I am not sure about the unnamed (`sub_...`) ones, but they are recognized as part of libc by IDA, just something to keep in mind.
...
seg000:0255 ; [0000000F BYTES: COLLAPSED FUNCTION sub_10255. PRESS CTRL-NUMPAD+ TO EXPAND] 🟡 two unknown functions recognized as part of libc by IDA
...
The executable is almost 7kB in size now. That's the overhead of libc on the game, close to 15%. But now let's look at the data segment.
...
This will have to wait for the next time though, since this post is already too long.
